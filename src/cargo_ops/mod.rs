...
impl TempProject {
    pub fn new<P: AsRef<Path>>(orig_manifest: P, orig_lockfile: P) -> CliResult<TempProject> {
        ...
        let mut parsed_manifest: Manifest =
            ::toml::from_str(&buf).expect("Cannot parse Cargo.toml");
        // TODO: figure out why this step is needed for serialization
        let mut dependencies = Table::new();
        for (dep_name, dep_detail) in &parsed_manifest.dependencies {
            match dep_detail {
                &Value::Table(ref t) => if let Some(&Value::String(ref s)) = t.get("version") {
                    dependencies.insert(dep_name.to_owned(), Value::String(s.clone()));
                } else {
                    panic!(
                        "Cannot find version for dependency {} in Cargo.toml",
                        dep_name
                    );
                },
                &Value::String(ref s) => {
        ...
        parsed_manifest.dependencies = dependencies;
    ...
    pub fn write_manifest_latest(&self) -> CliResult<()> {
        ...
        package.insert("version".to_owned(), version.clone());
        let mut dependencies = Table::new();
        for (dep_name, _) in &self.parsed_manifest.dependencies {
            dependencies.insert(dep_name.to_owned(), Value::String("*".to_owned()));
        }
        let mut bin = Table::new();
        bin.insert("name".to_owned(), Value::String("test".to_owned()));
        bin.insert("path".to_owned(), Value::String("test.rs".to_owned()));
